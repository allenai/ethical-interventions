...
parser.add_argument('--intv_type', help="intervention types, could be ['none', 'ethical', 'adversarial', 'irrelevant']", type=str, default="none")
parser.add_argument('--attribute', help="subject attribute group, ['religion', 'gender', 'ethnicity']", type=str, default="religion")
parser.add_argument('--converted', help="indicated if this data is converted from squad format (embeded with interventions). If so, there is intervention type in the key, need to be careful about the key split. 0 means original unqover data. 1 means converted", type=int, default=0, required=True)


def load_input(path, attribute, converted, intv_type = 'none'):
	print(f"do {intv_type} interventions for {attribute}")
	rs = []
	with open(f'./jyzhao/interventions/{attribute}_interventions.json', 'r') as intf:
		interventions = json.load(intf)
	with open(path, 'r') as f:
		...
		for key, ex in json_data.items():
			...
			#edited by jyzhao: concat the interventions to the questions: every original question will get a different intervention
			if converted:
				# print("Doing evaluation on the dev top 10k examples using Tao's model")
				subj0_cluster, subj1_cluster, subj0, subj1, tid, a_cluster, obj0, obj1, intv = key.strip().split('|')
			else:
				subj0_cluster, subj1_cluster, subj0, subj1, tid, a_cluster, obj0, obj1 = key.strip().split('|')	
			# print(a_cluster)
			# exit()
			if intv_type == 'none':
				#if it is original unqover dataset, remove the intv_type in the key.split & add 'none' to $rs$.
				if not converted:
					intv  = 'none'
				rs.append(((subj0_cluster, subj1_cluster), (subj0, subj1), tid, a_cluster, (obj0, obj1), context, choices, questions, intv))	
			else:
				intv_idx = 0
				for intv in interventions[intv_type][a_cluster]:
					if intv == 'NaN':
						print("there is NaN in this intv type")
						continue
					questions_aug = ["Note that, "+intv[0].lower()+intv[1:]+" " + questions[0], "Note that, "+intv[0].lower()+intv[1:]+ " " + questions[1]]
					rs.append(((subj0_cluster, subj1_cluster), (subj0, subj1), tid, a_cluster, (obj0, obj1), context, choices, questions_aug, intv_type+"_"+a_cluster+"-"+str(intv_idx)))
					intv_idx += 1
	return rs
...
def preprocess(opt, tokenizer, source):
	...
	for i, (scluster, spair, tid, acluster, opair, context, choices, questions, intv_id) in enumerate(tokenized):
...
def load_model(opt):
	...
	print("load model from {}".format(opt.hf_model))
...
def forward(opt, m, tok_idx, att_mask, type_idx):
	...
	if opt.hf_model == 'bert-large-uncased-whole-word-masking-finetuned-squad':
		start_scores, end_scores, hiddens = m(tok_idx, att_mask, type_idx, output_hidden_states=True)
	else:
		start_scores, end_scores, hiddens = m(tok_idx, att_mask, output_hidden_states=True)
	return torch.nn.functional.softmax(start_scores, dim=-1), torch.nn.functional.softmax(end_scores, dim=-1), hiddens
...
def main(args):
	...
	source = load_input(opt.input, opt.attribute, opt.converted, opt.intv_type)
	print("showing source examples:", source[7:9])
	print(f"in total, {len(source)} examples...")
	...
	while cnt < num_ex:
		...
		# forward pass
		with torch.no_grad():
			p_start, p_end, hiddens = forward(opt, m, tok_idx, att_mask, type_idx)
		# embs.extend(hiddens[0].detach().cpu().numpy())
		...
		for k in range(pred_span.shape[0]):
			...
			scluster, spair, tid, acluster, opair, context, choices, questions, intv_id = tokenized[row_id]
			
			keys = '|'.join([scluster[0], scluster[1], spair[0], spair[1], tid, acluster, opair[0], opair[1], intv_id])
			if keys not in rs_map:
				rs_map[keys] = {}
				rs_map[keys]['line'] = row_id
				rs_map[keys]['context'] = source[row_id][5]

			if 'q0' in rs_map[keys] and 'q1' in rs_map[keys]:
				print("duplicate keys:", keys)
			...
			# tks2save.append(batch_concated_toks[cnt+k])
	...
	print('saved {0} examples'.format(len(ls)))

	json.dump(rs_map, open(opt.output, 'w'), indent=4)
	# np.save(opt.output, embs)
	# json.dump(tks2save, open(opt.output+'_tks', 'w'), indent = 4)
