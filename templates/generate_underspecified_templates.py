...
parser.add_argument("--extra", help= 'whether to use extra info or not', action="store_true")
#
...
def load_extra(filename, act_repo):
    out = []
    with open(filename) as f:
        for line in f.readlines():
            ...
            if line.startswith('#'):
                ...
                out.append(line.replace('subj', 'subj0'))
    assert len(out) == len(list(act_repo.values())[0])
    cnt = 0
    for k,v in act_repo.items():
        for s in v:
            s = s.update({'extra': out[cnt]})
            cnt += 1
...
if opt.template_type == 'slot_act_map':
    ...
    if opt.extra:
        load_extra(os.path.join('word_lists/extra', 'gender'), act_repo)
